...
| 3 | **ACL discipline** | After every assignment to encrypted state, both `FHE.allowThis(handle)` and an appropriate `FHE.allow(handle, recipient)` are issued. The contract uses no deprecated `TFHE.*`, `Gateway*`, `requestDecryption`, or `fhevmjs` symbols. | `node check_acl.mjs <generated.sol>` exits 0 with `"status": "clean"` and `"totalViolations": 0`. The script reports both ACL discipline violations and v0.7/v0.8 deprecated-namespace usage. |
